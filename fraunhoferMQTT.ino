...
*/
   //mqtt @ganesh
    bool tcpInit()
    {
      return netSendCommand("AT+CIPOPEN=0,\"TCP\",\"137.251.108.210\",443\r", 3000);
    }

    bool tcpSend(const char* ip, uint16_t port, const char* data, unsigned int len)
    {
      //ip for fraunhofer mqtt broker --> 137.251.108.210 port--->443
      sprintf_P(buffer, PSTR("AT+CIPSEND=1,%u,\"%s\",%u\r"), len, ip, port);
...
public:
    ...
    bool regDataFeed()
    {
      ...
      Serial.print("#SERVER:");
      //char *ip = queryIP(SERVER_URL); #define SERVER_IP "137.251.108.210" defined in config.h for mqtt broker at fraunhofer
      ip=SERVER_IP; // 
      if (!ip) return false;
      Serial.println(ip);
      //strncpy(udpIP, ip, sizeof(udpIP) - 1);
      strncpy(tcpIP, ip, sizeof(tcpIP) - 1);
    ...
    */
    ...
    void transmitTCP()
    {
      ...
      if (!tcpSend(tcpIP, tcpPort, cache, cacheBytes)) {
        connErrors++;
      }else {
        connErrors = 0;
        connCount++;
